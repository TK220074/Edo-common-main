...
public static class TMProRubyUtil
{
	...
	{
#if true
		// (SAME Custom)リストに登録された単語は自動的にタグ付け
		Manager_CommonGroup commonM = Manager_CommonGroup.instance;
		for(int i = 0; i < commonM.dataM.list_WordsDictionary.list.Count; i++)
		{
			wordsDictionaryEntity data = commonM.dataM.list_WordsDictionary.list[i]; // 確認する単語のデータ

            // 特殊なふりがな配置を指定しているか
            if (data.ruby.Contains("</r>"))
			{
                // 指定している場合は、そのまま置き換え
                text = text.Replace(data.words, data.ruby);
            }
			else
			{
                // ルビ付きに置き換え
				string read = $"<r={data.ruby}>{data.words}</r>";
				text = text.Replace(data.words, read);
            }
        }
#endif
